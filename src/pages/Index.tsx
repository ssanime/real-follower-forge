  ...
  const fetchLatestManga = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("manga").select(`
          *,
          manga_genres(
            genres(name)
          )
  ...
  return <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <section className="py-12">
          ...
          {loading ? <div className="text-center text-muted-foreground">جاري التحميل...</div> : latestManga && latestManga.length > 0 ? <div className="grid grid-cols-2 gap-6 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
              {latestManga.map(manga => <Link key={manga.id} to={`/manga/${manga.id}`}>
                  <Card className="group overflow-hidden border-border/50 bg-card/50 backdrop-blur-sm transition-all hover:scale-105 hover:shadow-[var(--shadow-glow)]">
                    <div className="aspect-[3/4] overflow-hidden">
                      <img src={manga.cover_image_url || "/placeholder.svg"} alt={manga.title} className="h-full w-full object-cover transition-transform group-hover:scale-110" />
                    </div>
                    <div className="p-4">
                      ...
                      <div className="flex flex-wrap gap-1">
                        ...
                        {manga.manga_genres?.slice(0, 2).map((mg: any, i: number) => <Badge key={i} variant="outline" className="text-xs">
                            {mg.genres?.name}
                          </Badge>)}
                      </div>
                    </div>
